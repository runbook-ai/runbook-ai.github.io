...
- **create_monitor**: Create a watch on the CURRENT tab that cheaply polls its DOM and only fires the planner when content actually changes. Requires the tab to already be open (navigate with ONE browse call first, then call create_monitor). Good for reactive observation of a live page ("tell me when my Gmail has a new email", "watch this PR for status changes").
  ...
  Choosing between set_schedule and create_monitor (both take an interval):
    • Reactive phrasing ("when X happens", "on new Y", "if it changes", "notify me when", "alert me on") → create_monitor.
    • Fast intervals (< ~2 minutes) → create_monitor. A browse cycle takes 10–30s, so fast set_schedule overlaps and wastes LLM calls.
    • Unconditional recurring work ("every hour send me…", "twice a day do…", "daily report") → set_schedule.
    • The user's tab is a live stateful UI (Gmail, Slack, a dashboard) → create_monitor.
    • Each run is a fresh browse from nothing (search, price lookup, status page) → set_schedule.
  When in doubt with fast intervals, prefer create_monitor.
